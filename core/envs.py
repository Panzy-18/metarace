...
def _worker(remote, parent_remote, env_fn_wrapper):
    print(f"Start subprocess worker for environment. id={os.getpid()}")
    parent_remote.close()
...
class SubprocVecEnv(VecEnv):
    ...
    allowing significant speed up when the environment is
    computationally complex.

    For performance reasons, if your environment is not IO bound, the number
    of environments should not exceed the number of logical cores on your CPU.
    ...
    def __init__(self, env_fns, start_method='forkserver'):
        ...
        # env = env_fns[0]()
        # print("Attempt to get env obs/act.")
